...
private:
    static void writeToMyGraphFormat(const set<Node> &vertices, const set<Node> &interiors, const set<Edge> &edges,
                                     const string &path) {
        std::ofstream file;
        file.open(path);
        printNodes(vertices, "N,n", [](NodeData n) { return n.isHanging(); }, file);
        printNodes(interiors, "H,h", [](NodeData n) { return n.isToRefine(); }, file);
    ...
    static void printEdges(const set<Edge> &edges, std::ofstream &file) {
        int i = 0;
        for (auto edge : edges) {
            file << "E,e" << i++ << "," << std::get<0>(edge) << "," << std::get<1>(edge) << ","
                 << (std::get<2>(edge) ? "true" : "false") << std::endl;
        }
    }

    static void printNodes(const set<Node> &nodes, const string preambule, bool (*attributeChecker)(NodeData),
                           std::ofstream &file) {
        for (auto node : nodes) {
            file << preambule << node.first << "," << node.second.getCoords().getX() << ","
                 << node.second.getCoords().getY() << "," << node.second.getCoords().getZ() << ","
                 << (attributeChecker(node.second) ? "true" : "false") << std::endl;
        }
    }

    static void addEdge(set<Edge> &edges, const string &firstNodeId, const string &secondNodeId, bool border) {
    ...
    static string getNodeId(set<Node> &nodes, int &nodesIter, NodeData &data, optional<set<Node>> additionalNodesSet) {
    ...
    static void writeToFile(Graph &graph, const string &path) {
        ...
        for (auto node: graph) {
            ...
            if (!data.isHyperEdge) {
                ...
                string firstInteriorId = getNodeId(interiors, interiorsIter, data, optional<set<Node>>(vertices));
                for (const EdgeIterator &e : graph.edges(node)) {
                    GNode dstNode = graph.getEdgeDst(e);
                    NodeData dstNodeData = graph.getData(dstNode);
                    string secondNodeId = getNodeId(interiors, interiorsIter, dstNodeData, optional<set<Node>>(vertices));
